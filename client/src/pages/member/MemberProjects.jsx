...
const MemberProjects = ({ onNavigate }) => {
  ...
  const fetchProjects = async () => {
    try {
      setLoading(true);
      const res = await memberDashboardAPI.getProjects({
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] text-slate-800 flex flex-col relative overflow-x-hidden">
      ...
      <main className="flex-1 max-w-[1440px] w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 mt-1">
              Projects where you are assigned as an active team member.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200/80 pb-3">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              type="text"
              placeholder="Search participating projects..."
              value={searchQuery}
        ...
        {/* Project Cards Grid */}
        {loading ? (
          <div className="py-20 text-center text-xs text-slate-400">Loading your projects...</div>
          ...
          <div className="py-20 bg-white rounded-3xl border border-slate-200/80 text-center p-8 space-y-2">
            ...
            <p className="text-xs text-slate-400">
              You are not currently enrolled in any projects matching the selected filter.
            </p>
          </div>
        ) : (
                ...
                >
                  <div className="space-y-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="w-10 h-10 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
      ...
      {/* View-Only Project Details Modal */}
      {selectedProject && (
            ...
            <div className="space-y-5 text-xs text-slate-600 pt-2">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
                  <span className="text-[10px] text-slate-400 block font-medium">Target Deadline</span>
                ...
                <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
                  <span className="text-[10px] text-slate-400 block font-medium">Overall Progress</span>
                ...
                <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
                  ...
                  <span className="text-xs font-bold text-slate-800 mt-0.5 block">
                    {selectedProject.completedTaskCount || 0} / {selectedProject.taskCount || 0}
                  </span>
                </div>
              </div>
              ...
              {selectedProject.team && (
                <div>
                  ...
                  {selectedProject.team.lead && (
                    <div className="mb-2 p-2.5 rounded-xl border border-slate-100 bg-slate-50/50 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded-full bg-[#1e3a2f] text-white font-bold text-[10px] flex items-center justify-center">
                          {getInitials(selectedProject.team.lead.name)}
                        </div>
                        <div>
                          <span className="font-semibold text-slate-800 block text-xs">
                            {selectedProject.team.lead.name}
                          </span>
                          <span className="text-[10px] text-slate-400 block">
                            {selectedProject.team.lead.email}
                          </span>
                        </div>
                      </div>
                  ...
                  {selectedProject.team.members && selectedProject.team.members.length > 0 && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
                      {selectedProject.team.members.map((member) => (
                        <div
                          key={member.id}
                          className="flex items-center gap-2 p-2 rounded-xl border border-slate-100 bg-slate-50/30"
                        >
              ...
              <div>
                ...
                {(!selectedProject.tasks || selectedProject.tasks.length === 0) ? (
                  <p className="text-slate-400 italic py-2">No tasks recorded for this project.</p>
                ) : (
